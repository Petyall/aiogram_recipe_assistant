...
def cancel_adding_keyboard(type: str) -> InlineKeyboardMarkup:
    buttons = [
        [
            InlineKeyboardButton(
                text="Отменить добавление", callback_data=f"cancel_adding_{type}"
            )
        ]
    ]
...
def get_recipe_cancel_keyboard(is_author: bool = False) -> InlineKeyboardMarkup:
    buttons = [
        (
            [
                InlineKeyboardButton(
                    text="Редактировать рецепт", callback_data="edit_recipe"
                )
            ]
            if is_author
            else []
        ...
        [
            InlineKeyboardButton(text="Назад", callback_data="back_to_recipes"),
            InlineKeyboardButton(
                text="Отменить", callback_data="cancel_getting_recipe"
            ),
        ],
    ]
    keyboard = InlineKeyboardMarkup(inline_keyboard=buttons)
...
async def get_recipe_get_recipes_keyboard(category_id: int) -> InlineKeyboardMarkup:
    ...
    for recipe in recipes:
        keyboard.add(
            InlineKeyboardButton(
                text=recipe.article, callback_data=f"recipe_{recipe.id}"
            )
        )
    keyboard.adjust(1)
...
async def get_recipe_edit_recipe_keyboard() -> InlineKeyboardMarkup:
    fields = {"article": "Название", "ingredients": "Ингридиенты", "steps": "Шаги"}
    keyboard = InlineKeyboardBuilder()

    for field, name in fields.items():
        keyboard.add(InlineKeyboardButton(text=name, callback_data=f"field_{field}"))
    keyboard.adjust(1)
...
def get_recipe_confirm_keyboard(type: str) -> InlineKeyboardMarkup:
    text = {"edit": "изменить", "delete": "удалить"}
    buttons = [
        [
            InlineKeyboardButton(
                text=f"Да, {text[type]}", callback_data=f"confirm_{type}"
            )
        ],
